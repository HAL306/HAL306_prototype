...
public class PlayerMove : MonoBehaviour
{
    ...
    [SerializeField, Tooltip("地面とみなす最大角度")]
    [Range(0.0f, 90.0f)]
    ...
    private class AdvancedSettings
    {
        ...
        [Range(0.0f, 90.0f)]
        public float _minSlopeAngle = 10.0f;
        ...
        [SerializeField, Tooltip("ジャンプの先行入力猶予時間")]
        [Range(0.0f, 1.0f)]
        public float _jumpBufferDuration = 0.1f;
    }

    [Header("詳細設定")]
    [SerializeField, Tooltip("詳細設定")]
    ...
    public void OnJump(InputAction.CallbackContext context)
    {
        ...
        _jumpBufferTimer = _advanced._jumpBufferDuration;
    }
    ...
    private void Move()
    {
        if (_isGround && !_isJumping)
        {
            ...
            {
                // 移動速度を目標速度に近づける
                tangentVelocity = Mathf.MoveTowards(tangentVelocity, targetVelocity, _advanced._groundAcceleration * Time.fixedDeltaTime);
            }
        ...
        else
        {
            ...
            // 移動速度を目標速度に近づける
            _currentVelicity.x = Mathf.MoveTowards(_currentVelicity.x, targetVelocity, _advanced._airAcceleration * Time.fixedDeltaTime);
        }
    }
    ...
    private void AddGravity()
    {
        ...
        else
        {
            if (_wasGround && !_isJumping)
            {
                ...
                _currentVelicity.y = -_advanced._fallStartSpeed;
            }
            else
            {
    ...
    private void UpdateTimer(float delta)
    {
        ...
        else
        {
            if (_isGround)
            {
                _coyoteTimer = _advanced._coyoteTime;
            }
            else
            {
    ...
    private void CheckGround(Collision2D collision)
    {
        ...
        if (_isGround && !_wasGround &&
            minGroundAngle > _advanced._minSlopeAngle)
        {
