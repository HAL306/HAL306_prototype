...
namespace Game.Terrain
{
    ...
    public class TerrainPolygon : MonoBehaviour
    {
        ...
        private const int _maxEdgePoint = 10000;        // 最大エッジ生成数 (無限ループ対策)
        private const int _maxEdgeLoop = 100;           // 最大エッジループ生成数 (無限ループ対策)
        ...
        // 開始位置のセルを見つける
        private bool FindStartCell(out Vector2Int gridPos)
        {
        ...
        private bool GenerateEdgeLoop(Vector2Int startGridPos, out List<Vector2> edgeLoop)
        {
            ...
            for (int i = 0; i < _maxEdgePoint; ++i)
            {
                ...
                {
                    Debug.Log("地形のエッジ生成に失敗しました", _polygonCollider2D.gameObject);
                    return false;
                }
        ...
        private bool RebuildPolygon()
        {
            ...
            List<List<Vector2>> colliderPath = new List<List<Vector2>>();
            ...
            for (int i = 0; i < _maxEdgeLoop; ++i)
            {
                ...
                if (!FindStartCell(out startGridPos))
                {
                    if (i == 0)
                    {
                        // 一つも見つからなかったら失敗を返す
                ...
                // エッジループを生成
                List<Vector2> edgeLoop = new List<Vector2>();
                if (!GenerateEdgeLoop(startGridPos, out edgeLoop))
                    return false;

                colliderPath.Add(edgeLoop);
            }

            // コライダー形状を更新
            _polygonCollider2D.pathCount = colliderPath.Count;
            for (int i = 0; i < _polygonCollider2D.pathCount; ++i)
            {
                _polygonCollider2D.SetPath(i, colliderPath[i]);
            }
